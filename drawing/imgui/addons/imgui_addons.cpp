...
bool CustomSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float width, const char* format)
{
    ...
    // === FIX: draw filled track up to grab center, no grab_padding misalignment ===
    float grab_center_x = ImLerp(frame_bb.Min.x, frame_bb.Max.x, grab_t);

    ImGui::RenderNavHighlight(frame_bb, id);
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(colFrame), true, g.Style.FrameRounding);

    // Filled track from left edge to grab center
    if (grab_center_x > frame_bb.Min.x) {
        window->DrawList->AddRectFilled(
            frame_bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize),
            ImVec2(grab_center_x, frame_bb.Max.y - style.FrameBorderSize),
            ImGui::GetColorU32(colGrab),
            style.FrameRounding
        );
    }

    // Gradient shadow overlay on full track
    window->DrawList->AddRectFilledMultiColor(
        frame_bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize),
        frame_bb.Max - ImVec2(style.FrameBorderSize, style.FrameBorderSize),
        ImGui::GetColorU32(ImGuiCol_WindowShadow, 0),
        ImGui::GetColorU32(ImGuiCol_WindowShadow, 0),
